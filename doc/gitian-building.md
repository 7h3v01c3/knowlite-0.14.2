...
*Setup instructions for a Gitian build of Knowlite Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Knowlite
...
- [Building Knowlite Core](#building-knowlite-core)
...
Clone the git repositories for knowlite and Gitian.
...
in the knowlite repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Knowlite Core
----------------

To build Knowlite Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the knowlite repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/knowlite/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/knowlite-project/knowlite
...
For example:
```bash
URL=https://github.com/thrasher-/knowlite.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit knowlite=${COMMIT} --url knowlite=${URL} ../knowlite/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit knowlite=${COMMIT} --url knowlite=${URL} ../knowlite/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit knowlite=${COMMIT} --url knowlite=${URL} ../knowlite/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the knowlite git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../knowlite/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/knowlite
SIGPATH=/some/root/path/knowlite-detached-sigs

./bin/gbuild --url knowlite=${BTCPATH},signature=${SIGPATH} ../knowlite/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/knowlite-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/knowlite-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/knowlite-osx-build.assert
```
...
[knowlite-project/gitian.sigs.ltc](https://github.com/knowlite-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
